...
class PyramidBox(nn.Module):
    ...
    def forward(self, x):
        ...
        priors_boxes = PriorBox(size, feature_maps, self.cfg)
...
def build_detector(phase, cfg):
    base_, extras_, head_ = multibox(
        vgg(vgg_cfg, 12), add_extras((extras_cfg), 1024))
    lfpn_cpm = add_lfpn_cpm(lfpn_cpm_cfg)
    return PyramidBox(phase, base_, extras_, lfpn_cpm, head_, cfg)


def build_enhancer(n_blocks=6, stage_num=4, attention_type='eca'):
    return build_net_recurrent_scan(n_blocks, stage_num, attention_type)


def build_net_recurrent_scan(n_blocks, stage_num, attention_type):
    return ConvRNN(input_nc=3, output_nc=3, n_blocks=n_blocks, n_filters=24, use_dilation=True,
                   res_mode='Full', stage_num=stage_num, reduction=4, unit='GRU', attention_type=attention_type)


if __name__ == '__main__':
    inputs = Variable(torch.randn(1, 3, 640, 640))
    net = build_detector('train', cfg=2)
    print(net)
    out = net(inputs)
