...
void sequential()
{
    for (int countryIndex = 0; countryIndex < numberOfCountries; countryIndex++)
    {
        ...
        {
            std::ifstream in("C" + std::to_string(countryIndex) + "_P" + std::to_string(problemIndex) + ".txt");
            int id, score;
            while (in >> id >> score)
            {
                list.add(id, score);
...
void parallel(char* argv[])
{
    ...
    CustomQueue queue(100, numberOfReaders);
    ...
    int batchReminder = numberOfCountries * numberOfProblems % numberOfReaders;
    int start = 0;
    int end;
...
void reader(barrier<>& barrier, CustomQueue& queue, int start, int end)
{
    int countryIndex;
    int problemIndex;
    int id, score;
...
void createFiles()
{
    ...
    for (int countryIndex = 0; countryIndex < numberOfCountries; countryIndex++)
    {
        for (int problemIndex = 0; problemIndex < numberOfProblems; problemIndex++)
        {
            std::ofstream out("C" + std::to_string(countryIndex) + "_P" + std::to_string(problemIndex) + ".txt");
...
void verifyResuls()
{
    ...
    while (std::getline(inSec, lineSec))
    {
        ...
        if (lineSec != linePar)
        {
            std::cout << "Results are not the same at line " << lineSec << " - " << linePar << "\n";
            return;
        }
    }
}
